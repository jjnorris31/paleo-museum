          ...
          <v-col cols="12">
            <v-row no-gutters>
              ...
              <v-col cols="12"
                     class="mb-4">
                <v-data-table
                  :server-items-length="totalPreparators"
                  @click:row="openIndividualItem"
                  height="600px"
                  fixed-header
                  :items-per-page="25"
                  :options.sync="preparatorOptions"
...
export default {
  ...
  methods: {
    ...
    processPreparator() {
      ...
      this.preparator.fecha = this.getFmtEmptyField(this.preparator.fecha);
    ...
    async getPreparatorsFromDatabase() {
      ...
      await fetch('http://localhost:3000/forward', {
        ...
        if (res.hasMore) {
          ...
          this.totalPreparators = ((25 * (page + 1)));
        } else {
          this.totalPreparators = (25 * page);
        }
        this.preparators = res.items;
      }).catch(err => {
        console.log(err);
      });
