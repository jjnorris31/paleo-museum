    ...
    <v-row no-gutters
      ...
      <v-dialog v-model="formDialogActive"
        ...
        <v-row no-gutters
          ...
          <v-col cols="6">
            <h1 class="mb-6 text-h4 font-weight-medium" v-if="isAddingItem">Nueva especie</h1>
            <h1 class="mb-6 text-h4 font-weight-medium" v-if="isEditingItem">Editar especie</h1>
            ...
            <v-row no-gutters>
              <v-col cols="12"
                ...
                <h2 class="text-h4 font-weight-medium"
                    style="position: absolute; left: 0; margin-bottom: 26px">Especies</h2>
                <!-- ends title-->
  ...
  methods: {
    ...
    async saveSpecie() {
      if (this.$refs.specieForm.validate()) {
        this.setOverlayText('Guardando especie');
        this.showOverlay();
        this.processSpecie();
     ...
     */
    processSpecie() {
      this.specie.nombrecientifico = this.getFmtEmptyField(this.specie.nombrecientifico);
      this.specie.genero = this.getFmtEmptyField(this.specie.genero);
      this.specie.clase = this.getFmtEmptyField(this.specie.clase );
      this.specie.reino = this.getFmtEmptyField(this.specie.reino);
      this.specie.orden = this.getFmtEmptyField(this.specie.orden);
      this.specie.clase = this.getFmtEmptyField(this.specie.clase);
      this.specie.filum = this.getFmtEmptyField(this.specie.filum);
      this.specie.descripcion = this.getFmtEmptyField(this.specie.descripcion);
      this.specie.temporalidad = this.getFmtEmptyField(this.specie.temporalidad);
      this.specie.clado = this.getFmtEmptyField(this.specie.clado );
      this.specie.subclado = this.getFmtEmptyField(this.specie.subclado);
    },
    /**
     ...
     */
    async getSpeciesFromDatabase() {
      const {page, sortBy, sortDesc} = this.specieOptions;
