...
export default new Vuex.Store({
  ...
  actions: {
    ...
    async savePiece(context, piece) {
      try {
        return await fetch('http://129.146.80.82:3000/forward', {
          headers: new Headers({
    ...
    async saveUser(context, user) {
      try {
        return await fetch('http://129.146.80.82:3000/register', {
          headers: new Headers({
            'Authorization': `Bearer ${context.state.token}`,
            'Content-Type': 'application/json'
          }),
    ...
    async updatePiece(context, payload) {
      if (payload.photoFile) {
        ...
        try {
          await fetch('http://129.146.80.82:3000/images', {
            headers: new Headers({
              'Authorization': `Bearer ${context.state.token}`
            }),
            method: 'PUT',
    ...
    async deletePiece(context, piece) {
      if (piece.imagen) {
        try {
          await fetch(`http://129.146.80.82:3000/images/${piece.ncatalogo}`, {
            headers: new Headers({
              'Authorization': `Bearer ${context.state.token}`
            }),
            method: 'DELETE',
    ...
    async deletePerson(context, person) {
      ...
      if (person.imagen) {
        try {
          await fetch(`http://129.146.80.82:3000/images/${person.idp}`, {
            headers: new Headers({
              'Authorization': `Bearer ${context.state.token}`
            }),
            method: 'DELETE',
    ...
    async uploadPicture(context, imageInfo) {
      ...
      try {
        return await fetch("http://129.146.80.82:3000/images", {
          method: 'POST',
          body: formData
        });
      } catch (e) {
